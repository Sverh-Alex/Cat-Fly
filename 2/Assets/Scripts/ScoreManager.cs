...
public class ScoreManager : MonoBehaviour
{
    ...
    [SerializeField] private GameObject effectPSCoinInapp; // эффект монеток для кнопки инапа
    [SerializeField] private string inappProductId = "1"; // ID инап‑товара в каталоге Payments
    ...
    public void AddBonusMax() // Добавляет бонусные монеты за просмотр рекламы
    {
        ...
        //effectPSCoinInapp.Play(); // включаем эффект монеток
    ...
    private void OnPurchaseSuccess(string id)
    {
        ...
        YG2.SetState(id, 1); // Обновляем состояние товара (если используешь SetState для хранения)
                
        if (id == inappProductId) // Если это наш товар с ID "1" — начисляем монеты через AddBonusMax
        {
            AddBonusMax();
        }
    }
    public void BuyInapp()
    {
        ...
        YG2.PurchaseByID(inappProductId); // Запускаем фрейм оплаты товара "1"
    }
